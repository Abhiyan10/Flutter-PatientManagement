...
import 'package:group4_project/utils/custom_snackbar.dart';
...
class PatientController extends GetxController {
  ...
  String searchText = '';
  String searchCondition = 'Choose Condition';
  ...
  searchPatients(String val, String condition) {
    if (val.isEmpty && (condition.isEmpty || condition == 'Choose Condition')) {
      ...
      filteredPatients = allPatients
          .where((barber) =>
              (barber.name!.toLowerCase().contains(val.toLowerCase())))
          .toList();
    } else if (val.isEmpty &&
        (condition.isNotEmpty && condition != 'Choose Condition')) {
      filteredPatients = allPatients
          .where((barber) => (barber.conditions!
              .toLowerCase()
  ...
  deletePatientRecords(String id, context) async {
    ...
    if (res['success']) {
      patientRecords.removeAt(patientRecords
          .indexWhere((item) => item.id.toString() == id.toString()));
